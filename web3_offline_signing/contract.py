...
from web3_offline_signing.exceptions import (
    BadFunctionCallOutput,
)

from web3_offline_signing.utils.abi import (
    filter_by_type,
    filter_by_name,
...
from web3_offline_signing.utils.decorators import (
    combomethod,
)
from web3_offline_signing.utils.empty import (
    empty,
)
from web3_offline_signing.utils.events import (
    get_event_data,
)
from web3_offline_signing.utils.exception import (
    raise_from,
)
from web3_offline_signing.utils.filters import (
    construct_event_filter_params,
    PastLogFilter,
)
from web3_offline_signing.utils.normalizers import (
    BASE_RETURN_NORMALIZERS,
)
from web3_offline_signing.utils.validation import (
    validate_abi,
    validate_address,
)
...
class Contract(object):
    ...
    :meth:`web3_offline_signing.eth.Eth.contract` that takes compiled Solidity contract
    ...
    * Deploy a new smart contract using :py:meth:`Contract.deploy`
    ...
    def __init__(self, address=None):
        ...
        if self.web3 is None:
            raise AttributeError(
                ...
                '`web3_offline_signing.contract` interface to create your contract class.'
            )
    ...
    def deploy(cls, transaction=None, args=None, kwargs=None):
        ...
        .. code-block:: python
            ...
            >>> MyContract.deploy(
                transaction={
                    'from': web3_offline_signing.eth.accounts[1],
                    'value': 12345,
                },
                args=('DGD', 18),
            )
        ...
        txn_hash = cls.web3_offline_signing.eth.sendTransaction(deploy_transaction)
        return txn_hash
    ...
    def on(self, event_name, filter_params=None, *callbacks):
        ...
        log_filter = self.web3_offline_signing.eth.filter(event_filter_params)
    ...
    def pastEvents(self, event_name, filter_params=None, *callbacks):
        ...
        event_filter_params.setdefault('toBlock', self.web3_offline_signing.eth.blockNumber)
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.web3_offline_signing.eth.defaultAccount is not empty:
            estimate_transaction.setdefault('from', self.web3_offline_signing.eth.defaultAccount)
    ...
    def call(self, transaction=None):
        ...
        if self.web3_offline_signing.eth.defaultAccount is not empty:
            call_transaction.setdefault('from', self.web3_offline_signing.eth.defaultAccount)
    ...
    def transact(self, transaction=None):
        ...
        .. code-block:: python
            ...
            # Deposit to the `web3_offline_signing.eth.coinbase` account.
            >>> wallet.transact({'value': 12345}).deposit()
            '0x5c504ed432cb51138bcf09aa5e8a410dd4a1e204ef84bfed1be16dfba1b22060'
            # Deposit to some other account using funds from `web3_offline_signing.eth.coinbase`.
            >>> wallet.transact({'value': 54321}).deposit(web3_offline_signing.eth.accounts[1])
        ...
        if self.web3_offline_signing.eth.defaultAccount is not empty:
            transact_transaction.setdefault('from', self.web3_offline_signing.eth.defaultAccount)
...
def call_contract_function(contract,
    ...
    return_data = contract.web3_offline_signing.eth.call(call_transaction)
    ...
    try:
        ...
        is_missing_code_error = (
            ...
            contract.web3_offline_signing.eth.getCode(contract.address) in ACCEPTABLE_EMPTY_STRINGS
        )
...
def transact_with_contract_function(contract=None,
    ...
    txn_hash = contract.web3_offline_signing.eth.sendTransaction(transact_transaction)
    return txn_hash
...
def estimate_gas_for_function(contract=None,
    ...
    gas_estimate = contract.web3_offline_signing.eth.estimateGas(estimate_transaction)
    return gas_estimate
