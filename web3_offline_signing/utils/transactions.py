...
from web3_offline_signing.utils.compat import (
    Timeout,
)
...
from web3_offline_signing.utils.formatters import (
    apply_formatter_if,
...
TRANSACTION_DEFAULTS = {
    'gasPrice': lambda web3: web3_offline_signing.eth.gasPrice,
    'value': 0,
    'data': b'',
    'chainId': lambda web3: int(web3_offline_signing.net.version),
}
...
def wait_for_transaction_receipt(web3, txn_hash, timeout=120):
    with Timeout(timeout) as _timeout:
        while True:
            txn_receipt = web3_offline_signing.eth.getTransactionReceipt(txn_hash)
...
def get_block_gas_limit(web3, block_identifier=None):
    if block_identifier is None:
        block_identifier = web3_offline_signing.eth.blockNumber
    block = web3_offline_signing.eth.getBlock(block_identifier)
...
def get_buffered_gas_estimate(web3, transaction, gas_buffer=100000):
    ...
    gas_estimate = web3_offline_signing.eth.estimateGas(gas_estimate_transaction)
