...
class Filter(GreenletThread):
    ...
    def _run(self):
        ...
        while self.running:
            changes = self.web3_offline_signing.eth.getFilterChanges(self.filter_id)
            if changes:
    ...
    def stop_watching(self, timeout=0):
        self.running = False
        self.stopped = True
        self.web3_offline_signing.eth.uninstallFilter(self.filter_id)
        self.join(timeout)
...
class LogFilter(Filter):
    ...
    def get(self, only_changes=True):
        ...
        if only_changes:
            log_entries = self.web3_offline_signing.eth.getFilterChanges(self.filter_id)
        else:
            log_entries = self.web3_offline_signing.eth.getFilterLogs(self.filter_id)
...
class PastLogFilter(LogFilter):
    def _run(self):
        ...
        previous_logs = self.web3_offline_signing.eth.getFilterLogs(self.filter_id)
...
class ShhFilter(Filter):
    def _run(self):
        ...
        while self.running:
            changes = self.web3_offline_signing.shh.getFilterChanges(self.filter_id)
            if changes:
                for entry in changes:
                    for callback_fn in self.callbacks:
                        if self.is_valid_entry(entry):
    ...
    def stop_watching(self, timeout=0):
        self.running = False
        self.stopped = True
        self.web3_offline_signing.shh.uninstallFilter(self.filter_id)
        self.join(timeout)
