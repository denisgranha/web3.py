...
from web3_offline_signing.utils.datastructures import (
    HexBytes,
)
...
class EthModuleTest(object):
    def test_eth_protocolVersion(self, web3):
        protocol_version = web3_offline_signing.version.ethereum
    ...
    def test_eth_coinbase(self, web3):
        coinbase = web3_offline_signing.eth.coinbase
    ...
    def test_eth_hashrate(self, web3):
        hashrate = web3_offline_signing.eth.hashrate
        assert is_integer(hashrate)
        assert hashrate >= 0

    def test_eth_gasPrice(self, web3):
        gas_price = web3_offline_signing.eth.gasPrice
        assert is_integer(gas_price)
        assert gas_price > 0

    def test_eth_accounts(self, web3):
        accounts = web3_offline_signing.eth.accounts
        ...
        assert web3_offline_signing.eth.coinbase in accounts

    def test_eth_blockNumber(self, web3):
        block_number = web3_offline_signing.eth.blockNumber
        assert is_integer(block_number)
        assert block_number >= 0

    def test_eth_getBalance(self, web3):
        coinbase = web3_offline_signing.eth.coinbase
        balance = web3_offline_signing.eth.getBalance(coinbase)
    ...
    def test_eth_getTransactionCount(self, web3):
        coinbase = web3_offline_signing.eth.coinbase
        transaction_count = web3_offline_signing.eth.getTransactionCount(coinbase)

        assert is_integer(transaction_count)
        assert transaction_count >= 0

    def test_eth_getBlockTransactionCountByHash_empty_block(self, web3, empty_block):
        transaction_count = web3_offline_signing.eth.getBlockTransactionCount(empty_block['hash'])

        assert is_integer(transaction_count)
        assert transaction_count == 0

    def test_eth_getBlockTransactionCountByNumber_empty_block(self, web3, empty_block):
        transaction_count = web3_offline_signing.eth.getBlockTransactionCount(empty_block['number'])

        assert is_integer(transaction_count)
        assert transaction_count == 0

    def test_eth_getBlockTransactionCountByHash_block_with_txn(self, web3, block_with_txn):
        transaction_count = web3_offline_signing.eth.getBlockTransactionCount(block_with_txn['hash'])

        assert is_integer(transaction_count)
        assert transaction_count >= 1

    def test_eth_getBlockTransactionCountByNumber_block_with_txn(self, web3, block_with_txn):
        transaction_count = web3_offline_signing.eth.getBlockTransactionCount(block_with_txn['number'])

        assert is_integer(transaction_count)
        assert transaction_count >= 1

    def test_eth_getUncleCountByBlockHash(self, web3, empty_block):
        uncle_count = web3_offline_signing.eth.getUncleCount(empty_block['hash'])

        assert is_integer(uncle_count)
        assert uncle_count == 0

    def test_eth_getUncleCountByBlockNumber(self, web3, empty_block):
        uncle_count = web3_offline_signing.eth.getUncleCount(empty_block['number'])
    ...
    def test_eth_sign(self, web3, unlocked_account):
        signature = web3_offline_signing.eth.sign(unlocked_account, text='Message tö sign. Longer than hash!')
        assert is_string(signature)
        assert len(remove_0x_prefix(signature)) == 130

        # test other formats
        hexsign = web3_offline_signing.eth.sign(
            unlocked_account,
            hexstr='0x4d6573736167652074c3b6207369676e2e204c6f6e676572207468616e206861736821'
        )
        assert hexsign == signature

        intsign = web3_offline_signing.eth.sign(
            unlocked_account,
            0x4d6573736167652074c3b6207369676e2e204c6f6e676572207468616e206861736821
        )
        assert intsign == signature

        bytessign = web3_offline_signing.eth.sign(unlocked_account, b'Message t\xc3\xb6 sign. Longer than hash!')
        assert bytessign == signature

        new_signature = web3_offline_signing.eth.sign(unlocked_account, text='different message is different')
    ...
    def test_eth_sendTransaction(self, web3, unlocked_account):
        ...
        txn_hash = web3_offline_signing.eth.sendTransaction(txn_params)
        txn = web3_offline_signing.eth.getTransaction(txn_hash)
    ...
    def test_eth_call(self, web3, math_contract):
        ...
        call_result = web3_offline_signing.eth.call(txn_params)
    ...
    def test_eth_estimateGas(self, web3):
        coinbase = web3_offline_signing.eth.coinbase
        gas_estimate = web3_offline_signing.eth.estimateGas({
            'from': coinbase,
            'to': coinbase,
            'value': 1,
        })
    ...
    def test_eth_getTransactionByHash_contract_creation(self,
        ...
        transaction = web3_offline_signing.eth.getTransaction(math_contract_deploy_txn_hash)
        assert is_dict(transaction)
        assert transaction['to'] is None

    def test_eth_getTransactionByBlockHashAndIndex(self, web3, block_with_txn, mined_txn_hash):
        transaction = web3_offline_signing.eth.getTransactionFromBlock(block_with_txn['hash'], 0)
        assert is_dict(transaction)
        assert transaction['hash'] == HexBytes(mined_txn_hash)
    ...
    def test_eth_getTransactionReceipt_unmined(self, web3, unlocked_account):
        txn_hash = web3_offline_signing.eth.sendTransaction({
            'from': unlocked_account,
            'to': unlocked_account,
            'value': 1,
            'gas': 21000,
        ...
        receipt = web3_offline_signing.eth.getTransactionReceipt(txn_hash)
    ...
    def test_eth_newFilter(self, web3):
        filter = web3_offline_signing.eth.filter({})

        changes = web3_offline_signing.eth.getFilterChanges(filter.filter_id)
        assert is_list_like(changes)
        assert not changes

        logs = web3_offline_signing.eth.getFilterLogs(filter.filter_id)
        assert is_list_like(logs)
        assert not logs

        result = web3_offline_signing.eth.uninstallFilter(filter.filter_id)
        assert result is True

    def test_eth_newBlockFilter(self, web3):
        filter = web3_offline_signing.eth.filter('latest')
    ...
    def test_eth_newPendingTransactionFilter(self, web3):
        filter = web3_offline_signing.eth.filter('pending')
        assert is_string(filter.filter_id)
    ...
    def test_eth_uninstallFilter(self, web3):
        ...
        success = web3_offline_signing.eth.uninstallFilter(filter.filter_id)
        assert success is True

        failure = web3_offline_signing.eth.uninstallFilter(filter.filter_id)
