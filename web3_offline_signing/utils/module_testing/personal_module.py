...
class PersonalModuleTest(object):
    def test_personal_importRawKey(self, web3):
        actual = web3_offline_signing.personal.importRawKey(PRIVATE_KEY_HEX, PASSWORD)
        assert is_same_address(actual, ADDRESS)

    def test_personal_listAccounts(self, web3):
        accounts = web3_offline_signing.personal.listAccounts
    ...
    def test_personal_lockAccount(self, web3, unlocked_account):
        ...
        web3_offline_signing.personal.lockAccount(unlocked_account)

    def test_personal_unlockAccount_success(self,
                                            web3,
                                            unlockable_account,
                                            unlockable_account_pw):
        result = web3_offline_signing.personal.unlockAccount(unlockable_account, unlockable_account_pw)
        assert result is True

    def test_personal_unlockAccount_failure(self,
                                            web3,
                                            unlockable_account):
        result = web3_offline_signing.personal.unlockAccount(unlockable_account, 'bad-password')
        assert result is False

    def test_personal_newAccount(self, web3):
        new_account = web3_offline_signing.personal.newAccount(PASSWORD)
    ...
    def test_personal_sendTransaction(self,
                                      ...
                                      unlockable_account,
                                      unlockable_account_pw):
        assert web3_offline_signing.eth.getBalance(unlockable_account) > web3_offline_signing.toWei(1, 'ether')
        txn_params = {
            'from': unlockable_account,
            'to': unlockable_account,
            'gas': 21000,
            'value': 1,
            'gasPrice': web3_offline_signing.toWei(1, 'gwei'),
        }
        txn_hash = web3_offline_signing.personal.sendTransaction(txn_params, unlockable_account_pw)
        assert txn_hash
        transaction = web3_offline_signing.eth.getTransaction(txn_hash)
    ...
    def test_personal_sign_and_ecrecover(self,
        ...
        signature = web3_offline_signing.personal.sign(message, unlockable_account, unlockable_account_pw)
        signer = web3_offline_signing.personal.ecRecover(message, signature)
