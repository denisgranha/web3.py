...
class Web3ModuleTest(object):
    def test_web3_clientVersion(self, web3):
        client_version = web3_offline_signing.version.node
    ...
    def test_soliditySha3(self, web3, types, values, expected):
        if isinstance(expected, type) and issubclass(expected, Exception):
            with pytest.raises(expected):
                web3_offline_signing.soliditySha3(types, values)
            return

        actual = web3_offline_signing.soliditySha3(types, values)
    ...
    def test_is_connected(self, web3):
        assert web3_offline_signing.isConnected()
